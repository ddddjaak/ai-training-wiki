...
可以，但**强烈推荐使用 CC Switch**。如果不装 CC Switch，你需要手动设置两个环境变量：

```powershell
$env:ANTHROPIC_BASE_URL = "公司 new-api 平台地址"
$env:ANTHROPIC_AUTH_TOKEN = "你的 API Key"
```

每打开一个新终端都要重新设置（或者写入系统环境变量）。CC Switch 的价值在于自动管理这些环境变量、支持多供应商切换、可视化配置，省去手写和排查的成本。如果你只是临时测试、或者环境变量已经由 IT 通过组策略统一配置，可以不装。
